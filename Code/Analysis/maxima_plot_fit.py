...
def fit_parametric(n, m, c, q, k):
    ...
    return float(np.piecewise(n, [n < q, n >= q], [lin, exp]))
...
def plot_maxima_curve(filename, plot=True, fit_func=None, p0=[]):
    ...
    if plot:
        ...
        if fit_func:
            n = list(range(len(be)))
            params, _ = curve_fit(fit_func, n, be, p0)
            be_fit = fit_func(np.array(n), *params)
            plt.plot(n, be_fit, label=label)
            return time, be, params
...
plot_maxima_curve(files_to_compare[0], fit_func=fit_linear, p0=[1, 1]) # Rotational.
plot_maxima_curve(files_to_compare[1], fit_func=fit_parametric, p0=[1, 1, 15, 1]) # Parametric.
