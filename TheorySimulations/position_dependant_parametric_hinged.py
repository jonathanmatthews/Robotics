import Flail_redo_open_nogame as Sim
import pygame
import pygame.event
...
while t < 1000:
    ...
    """
np.savetxt('TimeArray_Parametric_Hinged_No_Damping.csv', t_array, delimiter = ',')
np.savetxt('SwingAngles_Parametric_Hinged_No_Damping.csv', swing_angles, delimiter = ',')
np.savetxt('MaxAngles_Parametric_Hinged_No_Damping.csv', max_angles, delimiter = ',')
np.savetxt('BodyPositions_Parametric_Hinged_No_Damping.csv', body_positions, delimiter = ',')
np.savetxt('LegPositions_Parametric_Hinged_No_Damping.csv', leg_positions, delimiter = ',') 

    """
    for event in pygame.event.get():
        if event.type == QUIT:
            #np.savetxt('TimeArray_Parametric_Hinged.csv', t_array, delimiter = ',')
            #np.savetxt('SwingAngles_Parametric_Hinged.csv', swing_angles, delimiter = ',')
            #np.savetxt('BodyPositions_Parametric_Hinged.csv', body_positions, delimiter = ',')
            #np.savetxt('LegPositions_Parametric_Hinged.csv', leg_positions, delimiter = ',')
            pygame.display.quit()
            pygame.quit()
