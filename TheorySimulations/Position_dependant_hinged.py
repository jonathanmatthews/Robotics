import Flail_redo_open_pygame as Sim
import pygame
import pygame.event
...
while t < 1000:
    ...
    """
np.savetxt('TimeArray_Rotational_Hinged_No_Damping.csv', t_array, delimiter = ',')
np.savetxt('SwingAngles_Rotational_Hinged_No_Damping.csv', swing_angles, delimiter = ',')
np.savetxt('MaxAngles_Rotational_Hinged_No_Damping.csv', max_angles, delimiter = ',')
np.savetxt('BodyPositions_Rotational_Hinged_No_Damping.csv', body_positions, delimiter = ',')
np.savetxt('LegPositions_Rotational_Hinged_No_Damping.csv', leg_positions, delimiter = ',')

    #print(swing_angles[-1])
#print(max_angle*180/np.pi)
    """
    for event in pygame.event.get():
        if event.type == QUIT:
            #np.savetxt('TimeArray_Rotational_Hinged.csv', t_array, delimiter = ',')
            #np.savetxt('SwingAngles_Rotational_Hinged.csv', swing_angles, delimiter = ',')
            #np.savetxt('BodyPositions_Rotational_Hinged.csv', body_positions, delimiter = ',')
            #np.savetxt('LegPositions_Rotational_Hinged.csv', leg_positions, delimiter = ',')
            pygame.display.quit()
            pygame.quit()
...
#print(max_angle*180/np.pi)
#print(np.exp(-2.56*np.deg2rad(0.0034)))
#print(np.exp(-2.56*0.0034))
