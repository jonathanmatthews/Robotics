...
//decoupled differential equations to be solved
...
int main()
{
    ...
    //Link output file
    ofstream outFile("double_hinged_flail_final_ind.txt");
    ...
    double COM_Y = (ym1*m1 + ym2*m2 + ym3*m3 + ym4*m4 + ym5*m5)/(m1+m2+m3+m4+m5);
    ...
    double start_ratio;
    dang=2.5*dang_min;
    double max_dang = 2*h/0.48;
    double changedang=max_dang*(h/0.02);
    //dang = 0;
    cout << "dangle: " << dang << "\n";
    /////////////////////////////////////////////
    ...
    double move_right_prev=false;
    double move_left_prev=false;
    ...
    for(int n=0; n<=N; n++){
        ...
        //COM
        //seat
        xm1 = XPosition(length,theta),    ym1 = YPosition(length,theta); //seat
        xm3 = x1 + r3*sin(t3),            ym3 = y1 + r3*cos(t3); //upper leg
        xm2 = x1 + r2*sin(t2),            ym2 = y1 + r2*cos(t2); //torso
        xm4 = x2 + r4*sin(t4),            ym4 = y2 + r4*cos(t4); //head
        xm5 = x3 + r5*sin(t5),            ym5 = y3 + r5*cos(t5); //lower leg

        COM_Y = ym1 - (ym1*m1 + ym2*m2 + ym3*m3 + ym4*m4 + ym5*m5)/(m1+m2+m3+m4+m5); //calculate centre of mass
        ...
        if(move_right_prev + move_right == 1){

            //dang=0;
        }
        if(move_left_prev + move_left == 1){

            //dang=0;
        }

        //check within limits
        if ( (right) && (term1 > 0.9*term1lim) ){

            move_right=false;
        }
        if ( (left) && (term1 < -0.9*term1lim) ){
        ...
        if (move_right){
            ...
            //////
            //dang+=changedang; //to approximate robot acceleration in real life
            //dang=dang_min;
        }
        ...
        term1dd = ((-((term1-term1_prev)/h)-term1d)/h); //acceleration of rotation
        term1d = -(term1-term1_prev)/h; //velocity of rotation
        ...
        ///trap rule to examine acceleration per cycle
        trap_now=term1dd;
    ...
    //user instructions
    printf ("Results have been output to a file called \"double_hinged_flail_final_ind.txt\"\n");
